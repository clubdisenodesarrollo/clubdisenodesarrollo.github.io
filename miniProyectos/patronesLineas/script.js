...
function draw(c, x, C, S, T, R, t) {
  ...
  for(j=1;j<w.length;j++){
    ...
    let z = kk[2];
     ...
     x.strokeStyle=x.fillStyle=`hsl(${t*j},50%,50%,4)`;
     x.lineTo(y,k+z)
    
    x.fillRect(y,k+z,0.2,0.9) /* Puntos de union, pixeles, cuadrados */
  }
  x.stroke()
  requestAnimationFrame(() => draw(c, x, C, S, T, R, t + 0.08));
}
draw(c, x, C, S, T, R, 500);
