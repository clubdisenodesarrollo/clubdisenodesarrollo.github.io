// --- Utilidades ---
function crearBoton(texto, id, onClick) {
...
function crearVolver(onClick) {
    ...
    btn.src = 'btn-volver.png'; // Usa tu imagen de volver
    btn.alt = 'Volver';
...
function showHome() {
  ...
  `;
    ...
    btnImg.src = 'btn-iniciar.png'; // Usa aquí tu imagen personalizada
    btnImg.alt = 'Iniciar';
...
function showNiveles(juego) {
    const app = document.getElementById('app');
    // Determina el fondo según el juego
    let fondo = '';
    switch (juego) {
    ...
    `;
    // Asigna eventos a los botones de nivel
...
function showEjercicioResta1() {
    const app = document.getElementById('app');
    // Genera 5 ejercicios aleatorios
    const ejercicios = [];
    for (let i = 0; i < 5; i++) {
        let a = Math.floor(Math.random() * 10) + 5; // 5 a 14
        let b = Math.floor(Math.random() * 5) + 1;  // 1 a 5
        if (b > a) [a, b] = [b, a]; // asegura que a >= b
    ...
    // Verificación automática al llenar todos los inputs
    ejercicios.forEach((ej, idx) => {
        ...
        input.addEventListener('input', () => {
            ...
            // Verifica si todos están correctos
