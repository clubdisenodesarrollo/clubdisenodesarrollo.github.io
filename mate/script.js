...
function crearCasita(onClick) {
  const btn = document.createElement('img');
  btn.src = 'casita.png';
  btn.alt = 'Ir a juegos';
  btn.className = 'casita-btn-img casita-bottom';
  btn.style.cursor = 'pointer';
  btn.onclick = onClick;
  return btn;
}
function mostrarModal(mensaje, onClose) {
  ...
  // Si es el mensaje de felicitaciones (ejercicio 2)
  if (typeof mensaje === 'string' && mensaje.toLowerCase().includes('felicidades')) {
    modal.innerHTML = `
      <div class="modal-content modal-felicidades">
        <img src="felicidades.png" alt="Felicidades" class="felicidades-img">
...
function showEjercicioSuma1() {
  ...
  navBtns.appendChild(crearVolver(() => showNiveles('suma'))); // o el nivel correspondiente

  // Solo en ejercicios, agrega también el botón casita:
  navBtns.appendChild(crearCasita(showGames));
...
function showEjercicioMulti1() {
  ...
  const container = app.querySelector('.container');
  container.appendChild(crearVolver(() => showNiveles('multiplicacion')));
  container.appendChild(crearCasita(showGames));
}
    ...
    drag.addEventListener('touchend', function (e) {
      ...
      dropzones.forEach(drop => {
        ...
        if (
          ...
          if (drag.dataset.resultado === drop.dataset.resultado) {
            drop.appendChild(drag);
            drag.setAttribute('draggable', 'false');
            drag.classList.add('multi2-correcto');
            aciertos++;
            if (aciertos === 6) {
              setTimeout(() => {
...
function showEjercicioFraccion1() {
  ...
  const container = app.querySelector('.container');
  container.appendChild(crearVolver(() => showNiveles('fraccion')));
  container.appendChild(crearCasita(showGames));
}
...
function showEjercicioFraccion2() {
  ...
  const dropzones = [
    { id: 'drop-2', correcto: 'drag-1' },
    { id: 'drop-1', correcto: 'drag-3' },
    { id: 'drop-3', correcto: 'drag-2' }
  ];
    ...
    drag.addEventListener('touchend', function (e) {
      ...
      dropzonesEls.forEach(drop => {
        ...
        if (
          ...
          // Solo permite un drop por zona
          if (drop.childNodes.length > 0) return;
          // Solo permite el drag correcto en el drop correcto
          if (drop.id === dropzones.find(dz => dz.correcto === drag.id).id) {
            drop.appendChild(drag);
            dropped = true;
          }
        }
  ...
  dropzonesEls.forEach((drop, idx) => {
    ...
    drop.addEventListener('drop', e => {
      ...
      if (drop.childNodes.length > 0) return;
      // Solo permite el drag correcto en el drop correcto
      if (drop.id === dropzones.find(dz => dz.correcto === dragElem.id).id) {
        drop.appendChild(dragElem);
        dragElem.setAttribute('draggable', 'false');
